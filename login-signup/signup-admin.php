...
include("functions.php");
    ...
    if($_SERVER['REQUEST_METHOD']=="POST"){
        ...
        if(!empty($admin_id)&&!empty($password) ){
            $check_username = mysqli_query($con, "SELECT admin_id FROM admin WHERE admin_id = '$admin_id'");
            if(mysqli_num_rows($check_username)>0){
                echo('Username Already Exist');
                
            }
            else{
                $query ="insert into admin(admin_id,password,name,email) VALUES('$admin_id','$password','$name','$email')";
    ...
    <body>
        ...
        <div id="box">
            <form method="post">
                ...
                <input id="text" type="text" name="admin_id" placeholder="Enter username">
                <br>
                <br>
